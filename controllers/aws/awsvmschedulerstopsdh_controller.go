...
// AWSVMSchedulerStopsdhReconciler reconciles a AWSVMSchedulerStopsdh object
type AWSVMSchedulerStopsdhReconciler struct {
	client.Client
	Scheme *runtime.Scheme
	Log    logr.Logger
}

//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstopsdhs,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstopsdhs/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstopsdhs/finalizers,verbs=update
//+kubebuilder:rbac:groups=batch,resources=cronjobs;jobs,verbs=get;list;watch;create;update;patch;delete

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the AWSVMSchedulerStopsdh object against the actual cluster state, and then
...
func (r *AWSVMSchedulerStopsdhReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	awsVMScheduler := &awsv1.AWSVMSchedulerStopsdh{}
	...
	if err != nil && errors.IsNotFound(err) {
		...
		cron := r.cronJobForAWSVMSchedulerStopsdh(awsVMScheduler)
	...
	if !reflect.DeepEqual(currentInstanceIds, awsVMScheduler.Status.VMStopStatus) { ////
...
func (r *AWSVMSchedulerStopsdhReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&awsv1.AWSVMSchedulerStopsdh{}).
...
func (r *AWSVMSchedulerStopsdhReconciler) cronJobForAWSVMSchedulerStopsdh(awsVMScheduler *awsv1.AWSVMScheduler) *batchv1.CronJob {
	...
	cron := &batchv1.CronJob{
		ObjectMeta: v1.ObjectMeta{
			...
			Labels:    AWSVMSchedulerStopsdhLabels(awsVMScheduler, "awsVMScheduler"),
		},

...
// AWSVMSchedulerStartsdhReconciler reconciles a AWSVMSchedulerStartsdh object
type AWSVMSchedulerStartsdhReconciler struct {
	client.Client
	Scheme *runtime.Scheme
	Log    logr.Logger
}

//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstartsdhs,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstartsdhs/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=aws.xyzcompany.com,resources=awsvmschedulerstartsdhs/finalizers,verbs=update
//+kubebuilder:rbac:groups=batch,resources=cronjobs;jobs,verbs=get;list;watch;create;update;patch;delete

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the AWSVMSchedulerStartsdh object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.13.0/pkg/reconcile
func (r *AWSVMSchedulerStartsdhReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {

	//_ = log.FromContext(ctx)
	log := ctrllog.FromContext(ctx)
	//log := r.Log.WithValues("AWSVMScheduler", req.NamespacedName)
	log.Info("Reconciling AWSVMSchedulerStartsdh")

	// Fetch the AWSVMSchedulerStartsdh CR
	//awsVMScheduler, err := services.FetchAWSVMSchedulerCR(req.Name, req.Namespace)

	// Fetch the AWSVMSchedulerStartsdh instance
	awsVMScheduler := &awsv1.AWSVMSchedulerStartsdh{}
	...
	if err != nil && errors.IsNotFound(err) {
		...
		cron := r.cronJobForAWSVMSchedulerStartsdh(awsVMScheduler)
...
func (r *AWSVMSchedulerStartsdhReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&awsv1.AWSVMSchedulerStartsdh{}).
		Owns(&batchv1.CronJob{}).
		Complete(r)
}

// CronJob Spec
func (r *AWSVMSchedulerStartsdhReconciler) cronJobForAWSVMSchedulerStartsdh(awsVMScheduler *awsv1.AWSVMSchedulerStartsdh) *batchv1.CronJob {

	cron := &batchv1.CronJob{
		ObjectMeta: v1.ObjectMeta{
			Name:      awsVMScheduler.Name,
			Namespace: awsVMScheduler.Namespace,
			Labels:    AWSVMSchedulerStartsdhLabels(awsVMScheduler, "awsVMScheduler"),
		},
...
func AWSVMSchedulerStartsdhLabels(v *awsv1.AWSVMSchedulerStartsdh, tier string) map[string]string {
	return map[string]string{
		"app":                       "AWSVMSchedulerStartsdh",
		"AWSVMSchedulerStartsdh_cr": v.Name,
		"tier":                      tier,
	}
}
